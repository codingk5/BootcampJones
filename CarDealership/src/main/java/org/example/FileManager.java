...
public class FileManager {
    ...
    public static void saveDealership(Dealership dealership) {
        try {
            ...
            // Write header row
            String headerRow = String.format("%s|%s|%s%n",
                    dealership.getName(),
                    dealership.getAddress(),
                    dealership.getPhone());
            fileWriter.write(headerRow);

            // Write each vehicle row
            for (Vehicle vehicle : dealership.getAllVehicle()) {
                String row = String.format("%d|%d|%s|%s|%s|%s|%d|%.2f%n",
                        vehicle.getVin(),
                        vehicle.getYear(),
                        vehicle.getMake(),
                        vehicle.getModel(),
            ...
            fileWriter.close();
            System.out.println("Dealership saved to inventory2.csv");

        } catch (IOException ex) {
            System.out.println("Not saved to inventory: " + ex.getMessage());
        }
    }
